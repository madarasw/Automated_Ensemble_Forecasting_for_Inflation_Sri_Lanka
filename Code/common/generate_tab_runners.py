"""Generate Code/<tab_name>/<tab_name>.py for every tab in Code/experiments.csv except
Tab22_Baselines (which has its own hand-written runner - no AutoGluon, no GPU, a
genuinely different shape of script). Every generated file is the SAME thin wrapper as
Tab05_HCPI_EM2/Tab05_HCPI_EM2.py (task 8: "generated from one template" so there's no
20x hand-maintenance burden) - only TAB_NAME and EVAL_METRIC_CODE differ.

Run this after Code/common/update_plan_for_local.py + regenerate_configs.py, whenever
experiments.csv's tab list changes.
"""
...
TEMPLATE = '''"""{tab_name} - {index_desc}, {eval_metric} ({metric_name}) as the ensemble-fitting objective.

Reads Code/experiments.csv filtered to tab_name == "{tab_name}", loops every row (train
...
Results/Results_{tab_name}.xlsx plus a run_config.json per run.
...
Code/common/config_{tab_name}.yaml, not hardcoded here - CLI flags override the YAML.

Run: python Code/{tab_name}/{tab_name}.py [--exp-id <id>] [--set S1] [--frequency Monthly]
...
def main() -> None:
    ...
    tabs: dict[str, dict] = {}
    for row in rows:
        tabs.setdefault(row["tab_name"], row)

    written = []
    for tab_name, sample_row in sorted(tabs.items()):
        if tab_name == "Tab22_Baselines":
            continue  # hand-written, different shape (no AutoGluon/GPU) - never generated
        out_dir = REPO_ROOT / "Code" / tab_name
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / f"{tab_name}.py"
        content = TEMPLATE.format(
            tab_name=tab_name,
            eval_metric=sample_row["eval_metric"],
            metric_name=sample_row["metric_name"],
            index_desc=sample_row["index_name"],
        )
    ...
    print(f"\nGenerated {len(written)} tab runner scripts.")
