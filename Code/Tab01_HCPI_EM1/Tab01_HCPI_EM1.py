"""Tab01_HCPI_EM1 - Headline Consumer Price Index, EM1 (SELECTED (to be determined)) as the ensemble-fitting objective.

Reads Code/experiments.csv filtered to tab_name == "Tab01_HCPI_EM1", loops every row (train
...
Results/Results_Tab01_HCPI_EM1.xlsx plus a run_config.json per run.
...
Code/common/config_Tab01_HCPI_EM1.yaml, not hardcoded here - CLI flags override the YAML.

Run: python Code/Tab01_HCPI_EM1/Tab01_HCPI_EM1.py [--exp-id <id>] [--set S1] [--frequency Monthly]
...
TAB_NAME = "Tab01_HCPI_EM1"
EVAL_METRIC_CODE = "EM1"
